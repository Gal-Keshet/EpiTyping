...
include { OUTPUT_ASE; OUTPUT_DNA_TABLE; LOI_MATRIX_DNA_INTEGRATION } from "$params.module_dir/dna_integration.nf"
...
workflow EPITYPING {
    ...
    xci_out_ch = XCI_MATRIX(var_x_table_ch, counts_ch.collect(), params.x_escape)

    if(params.dna_reference != "" ) {

      dna_vcf_ch = DNA()

      dna_ch = dna_vcf_ch.collect()

      ase_ch = OUTPUT_ASE(recalibrated_ch, dna_ch, params.ref, "${params.ref}.fai", params.ref_dict)

      dna_table_ch = OUTPUT_DNA_TABLE(dna_ch)

      ase_combined_ch = ase_ch.collect()

      loi_matrix_dna_integrated_ch = LOI_MATRIX_DNA_INTEGRATION(dna_table_ch, ase_combined_ch, counts_files_ch, params.imprinted_genes)
    }
    else {
