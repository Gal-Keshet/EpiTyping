...
process XCI_MATRIX {
    ...
    tpm_y <- tpm_y[-c(1:4)][colnames(allele_table)]
    ...
    output_table <- data.frame(X_status = apply(scores, 1, function(x) {
     ...
     else if(x['x_vs_a_allelic_ratios'] < 0.12 & x['Y_av_exp'] < 5) return('XaXi')
     else if(x['x_vs_a_allelic_ratios'] < 0.12 & x['Y_av_exp'] > 5) return('Male')
    }))
